...
public:
    vector<string> addOperators(string num, int t) {
        vector<string> ans;
        string path = string(num.length() * 2, ' ');
        helper(ans, num, /*pos=*/0, path, /*len=*/0, 0, 0, t);
        return ans;
    }
    void helper(vector<string>& ans, const string& num, int pos, string& path, int len, int current_value, int previous_value, int target) {
        if(pos == num.length()) {
            if(current_value == target) ans.push_back(path.substr(0, len)); 
            return;
        }
        long number = 0;
        int moving_pos = pos, moving_len = len;
        if (pos != 0) moving_len++; // 1st number does not have leading op. the following numbers have leading op.
